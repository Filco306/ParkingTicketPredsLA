...
class Plotter:
    ...
    def create_bar_plot(self,col,df, top_x):
        ...
        bar_source = ColumnDataSource(data=dict(indices= indices,
                                                heights= heights.loc[heights.reset_index().index < top_x]))
        TOOLTIPS = [
            (col,"@indices"),
            ("Frequency","@heights")
        ]
        bar_plot = figure(x_range=[str(x) for x in list(indices)][:top_x], plot_height=250, title=col,tooltips=TOOLTIPS)
    ...
    def test_hist(self, doc):
        ...
        df = df.groupby(['Location','Latitude','Longitude']).count().reset_index()[['Location','Latitude','Longitude','Exact issuing time']]
        ...
        source = ColumnDataSource(data=dict(lon=df['Longitude'], lat=df['Latitude'],
                    loc = df['Location'],
                    amount_of_tickets = df['Exact issuing time'],
                    colors_chosen = [self.colours[x] for x in self.km.predict(df[['Latitude','Longitude']])],
                    x = lon,
                    y = lat,
                    amt=3*np.log1p(df['Exact issuing time'])/np.min(np.log1p(df['Exact issuing time']))))
        TOOLTIPS = [
            ("Location","@loc"),
            ("Latitude","@lat"),
            ("Longitude","@lon"),
        ...
        r = p.scatter(x="x", y="y", size="amt", source=source, color="colors_chosen", alpha=0.6)
        ...
        starting_date = DatePicker(title="Date Range: ", min_date=self.dh.full_df['date issued'].min(), max_date=self.dh.full_df['date issued'].max(), value=min_date)
        ending_date = DatePicker(title="Date Range: ", min_date=self.dh.full_df['date issued'].min(), max_date=self.dh.full_df['date issued'].max(), value=max_date)
        max_amount_of_points = Slider(title="Number of points in scatter: ", start=500, end = 1000000, step=500, value = 1000)
        nClusters = Slider(title="Number of clusters (may take some time to retrain): ", start = 2, end = 15, step=1, value = 4)
        multi_select = MultiSelect(title="Which cars makes to include:", value=list(self.dh.full_df.Make.value_counts().index),
                           options=list(self.dh.full_df.Make.value_counts().index))

        select_bar_variable = Select(title="",value=default_bar_var, options=[('Make','Make'), ('Violation Description','Violation Description'), ('Agency','Agency'), ('STR_NM','Street')])
        show_top = Slider(title="Show top x in bar chart: ", start = 5, end = 200, step=5, value = default_top)
        submit_button = Button(label="Update map")
        def select_data():
            print("BEFORE MINDATE")
            mindate = starting_date.value#date.fromtimestamp(starting_date.value / 1e3)
            maxdate = ending_date.value#date.fromtimestamp(ending_date.value / 1e3)
            df = self.dh.full_df
            print("HERE NOW ")
            print(maxdate)
            print(mindate)
            df = df.loc[(df['date issued'] >= mindate) & (df['date issued'] <= maxdate) & (df.Make.isin(multi_select.value))]
            print("Here, selecting data")
            print("max amt of points is ")
            print(max_amount_of_points.value)
            print(type(max_amount_of_points.value))
            self.bar_var_chosen = select_bar_variable.value
            bar_source_new, _ = self.create_bar_plot(self.bar_var_chosen,df,show_top.value)
            indices = [str(x) for x in list(df[self.bar_var_chosen].value_counts().index)]
            bar_plot.x_range.factors = [str(x) for x in list(indices)][:show_top.value]
            df = df.groupby(['Location','Latitude','Longitude']).count().reset_index().sort_values(by="Exact issuing time",ascending=False)
            return bar_source_new, df.loc[df.index < max_amount_of_points.value,['Location','Latitude','Longitude','Exact issuing time']]
        ...
        controls = [starting_date,ending_date,max_amount_of_points,nClusters,multi_select, submit_button]
