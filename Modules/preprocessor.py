...
class Preprocessor:
    ...
    """
    def convert_coordinates(self,df, remove_na_coords = True, longitude_box = [-120,-115], latitude_box = [32,36]):
        # Copied from another Kaggle user, Need to remember who
        ...
        long,lat = transform(Proj(pm, preserve_units = True), Proj("+init=epsg:4326"), x_in,y_in)
        df.loc[:,'Latitude'] = lat
        df.loc[:,'Longitude'] = long

        # Ugly hardcoding: make all positive latitudes negative, make all negative longitudes positive
        df.loc[df['Latitude'] < 0, "Latitude"] = df.loc[df['Latitude'] < 0, "Latitude"]*(-1)
        df.loc[df['Longitude'] > 0, "Longitude"] = df.loc[df['Longitude'] > 0, "Longitude"]*(-1)

        df = df.loc[(df['Latitude'] >= latitude_box[0]) & (df['Latitude'] <= latitude_box[1]) & (df['Longitude'] >= longitude_box[0]) & (df['Longitude'] <= longitude_box[1])]
    ...
    def convert_date_to_hourly(self,df):
        ...
        df['Exact issuing time'] = (df['Issue Date'].astype("str").str.slice(stop=11) + df['Issue time'].astype("str")).astype("datetime64[ns]")
    ...
    """
        Will impute, probabilistically, missing values for a specific column, based on other values.
        This is a very simple one, and a better approach would probably be to impute while regarding other columns as well.
    """
    def impute_probabilistically(self,df,col_to_impute, continuous = True):
        from scipy.stats import gaussian_kde
        density = gaussian_kde(df[col].dropna())
        x = np.arange(0,data[col].dropna().max())
    ...
    def generate_grid_points(self, x_dim, y_dim, time_start, time_end, time_step = '1H'):
        ...
        n = pd.MultiIndex.from_product([all_time_sections, x, y], names = ["Time section", "grid_x",'grid_y'])
        n = n.to_frame()
        n.columns = ["1","2","3"]
    ...
    """
    def full_preprocessing(self,raw_df):
        ...
        df.loc[df['is_imputed'] == True, "Latitude"] = df.loc[df['is_imputed'] == True, "LAT"]
        df.loc[df['is_imputed'] == True, "Longitude"] = df.loc[df['is_imputed'] == True, "LON"]
        df = df.loc[df.Latitude != 99999.0]
        df.drop(['HSE_DIR_CD_y',
         'HSE_NBR_y',
         'STR_SFX_CD_y',
         'STR_NM_y',
         'LAT',
         'LON',
         'is_imputed'],axis=1,inplace=True)
        df.columns = ['Ticket number',
         'Issue Date',
         'Issue time',
         'Meter Id',
         'Marked Time',
    ...
    def get_street_name(self,x):
        ...
        return " ".join([s for s in x['Location'].split() if s not in to_check and str(x['HSE_NBR']) not in s[:len(str(x['HSE_NBR']))]])
    ...
    def impute_coordinates(self, df, df_has_loc_info = True):
        ...
        if df_has_loc_info == False:
            locations = self.fix_locations(df)
        else:
            locations = df[["Ticket number", "HSE_DIR_CD", "HSE_NBR", "STR_SFX_CD", "STR_NM"]]
        types = {"HSE_DIR_CD": "str",
            "HSE_NBR": "int64",
            "STR_SFX_CD": "str",
            "STR_NM": "str"}
        for col in types:
        ...
        addresses_full = addresses_full.applymap(lambda x: x.strip() if isinstance(x, str) else x)
        locations = locations.applymap(lambda x: x.strip() if isinstance(x, str) else x)
        a = addresses_full[["HSE_DIR_CD","HSE_NBR","STR_SFX_CD","STR_NM","LAT","LON"]].merge(locations, on = ["HSE_DIR_CD","HSE_NBR","STR_SFX_CD","STR_NM"])
        b = addresses_full[["HSE_DIR_CD","HSE_NBR","STR_SFX_CD","STR_NM","LAT","LON"]].merge(locations, on = ["HSE_NBR","STR_SFX_CD","STR_NM"])
    ...
    def fix_locations(self, df):
        ...
        locations['HSE_DIR_CD'] = locations['Location'].str.extract(r'(\bE\b|\bN\b|\bW\b|\bS\b)')
        locations['HSE_NBR'] = locations['Location'].apply(lambda x : [s for s in x.split() if re.search(r"\d+", s) is not None and (any([y in s for y in ["ST","RD","ND","TH"]]) == False)])
        locations['HSE_NBR'] = locations['HSE_NBR'].apply(lambda x : x[0] if len(x) > 0 else "").str.extract(r'(\d+)')
        locations['STR_SFX_CD'] = locations['Location'].str.extract(r"(\b"+"\\b|\\b".join(list(addresses_full.STR_SFX_CD.value_counts().index))+")")
    ...
    def extract_more_coordinates(self,df, df_has_loc_info = True):
        """
            Tries to extract as many coordinates as possible, and adds these new ones to the main data frame.
        """
        ...
        df_wo_coords = df_wo_coords.merge(locations[['Ticket number','LAT','LON']],on="Ticket number", how = "left")
        df_wo_coords = df_wo_coords.drop(["Latitude","Longitude"],axis=1).rename({"LAT":"Latitude","LON":"Longitude"},axis=1)
        print(df_wo_coords.head())
        df_wo_coords.dropna(subset=["Latitude","Longitude"],inplace=True)
        return pd.concat([df_wo_coords,df],axis=0).drop_duplicates(subset=["Ticket number"],keep="first")


    def fill_nas(self,df):
        str_cols = ['RP State Plate',
         'Plate Expiry Date',
         'VIN',
         'Make',
         'Body Style',
